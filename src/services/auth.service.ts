...
export const loginWithGoogle = () => {
  ...
  window.location.href = `${api.uri}/auth/google`;
};

// Handle OAuth callback and login
export const handleOAuthCallback = async (token: string) => {
  try {
    const response = await axios.post(`${api.uri}/users/oauth/login?token=${token}`);
    
    if (response.data) {
      // Create a token for OAuth users (different from basic auth)
      const oauthToken = Buffer.from(`${response.data.username}:oauth_user`, 'utf8').toString('base64');
      localStorage.setItem("aToken", oauthToken);
      localStorage.setItem("user", JSON.stringify(response.data));
      return response.data;
    }
  } catch (error) {
